...
TEST(SampleCovariance, SqrtInf)
{
  ...
  auto stdev = cbr::sample_covariance(data, [](auto el) {return el;});
  auto sqrtinf = cbr::sqrt_information(stdev, 1e-2);

  ASSERT_LE(((sqrtinf * sqrtinf).inverse() - stdev).norm(), 1e-9);

  Eigen::Matrix3d stdev_singular;
  stdev_singular << 2, 1, 0, 1, 2, 0, 0, 0, 0;

  Eigen::Matrix3d stdev_fixed;
  stdev_fixed << 2, 1, 0, 1, 2, 0, 0, 0, 0.01;

  auto sqrtinf_singular = cbr::sqrt_information(stdev_singular, 1e-2);
  ASSERT_LE(((sqrtinf_singular * sqrtinf_singular).inverse() - stdev_fixed).norm(), 1e-9);
}

TEST(SampleCovariance, VarSqrtInf)
{
  ...
  auto sqrtI = cbr::variable_sqrt_information(
    x, data,
    [](const auto & x, const auto & el) {
      return (x - el).eval();
    }
  );

  ASSERT_GE(sqrtI.trace(), 0);
}
