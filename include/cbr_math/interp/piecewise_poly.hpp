...
/**************************************************************************
 * \brief Class for a scalar piecewise polynomial with Lie group image
 **************************************************************************/
template<typename Group_>
class PiecewisePolyLie : public pp_details::PiecewisePolyBase
{
public:
  using row_t = Eigen::Matrix<double, 1, Eigen::Dynamic>;
  using matrix_t = Eigen::MatrixXd;
  ...
  PiecewisePolyLie() = delete;
  PiecewisePolyLie(const PiecewisePolyLie &) = default;
  PiecewisePolyLie(PiecewisePolyLie &&) = default;
  PiecewisePolyLie & operator=(const PiecewisePolyLie &) = default;
  PiecewisePolyLie & operator=(PiecewisePolyLie &&) = default;
  ~PiecewisePolyLie() = default;

  /**
   * @brief Lie-valued piecewise polynomial
   *
   * @tparam T1 PiecewisePoly::row_t
   * @tparam T2 container_t<PiecewisePoly::matrix_t>
   * @tparam T3 container_t<LieType>
   * @param breakPoints range breakpoints of shape 1xN
   * @param coefLists polynomial coefficients of shape MxDoFxN
  ...
  : pp_details::PiecewisePolyBase(
      ...
      search),
    coefLists_{std::forward<T2>(coefLists)},
    poses_{std::forward<T3>(poses)}
  {
    if (length_ != poses_.size()) {
      throw std::invalid_argument("breakPoints and poses must have the same number of elements.");
    }

    orders_.reserve(Dimension);
    ...
    for (const auto & coefList : coefLists_) {
      ...
      if constexpr (Dimension == 1) {
        ...
        extremities_.back =
          evalPolyDer<GroupScalar>(
          breakPoints_[length_ - 1],
          length_ - 2,
          1,
          coefList,
          orders_.back());
      } else {
        extremities_.front[i] =
          evalPolyDer<GroupScalar>(
          breakPoints_[0],
          0,
          1,
    ...
    if constexpr (Dimension > 1)
    {
      ...
      for (std::size_t i = 0; i < Dimension; i++) {
        omegaVal[i] =
          evalPoly<GroupScalar>(breakPoints_[length_ - 1], length_ - 2, coefLists_[i], orders_[i]);
      }

      const auto Dexp = lie::dr_exp<Group>(omegaVal);
      extremities_.back = Dexp * extremities_.back;
    }
  }

  Group val(const GroupScalar x) const
  {
    const auto [pieceIdx, flag] = findPosition(static_cast<double>(x));
    if (flag == 0) {
      Tangent omega;
      if constexpr (Dimension == 1) {
        omega = evalPoly<GroupScalar>(x, pieceIdx, coefLists_[0], orders_[0]);
      } else {
        for (std::size_t i = 0; i < Dimension; i++) {
          omega[i] = evalPoly<GroupScalar>(x, pieceIdx, coefLists_[i], orders_[i]);
        }
      }
      return poses_[pieceIdx] * Group::exp(omega);
    } else {
      return extrapPolyLie(flag, x);
    }
  }

  vector_aligned<Group>
  vals(const std::vector<GroupScalar> & xs) const
  {
    vector_aligned<Group> out;
    out.reserve(xs.size());
    for (const auto & x : xs) {
      out.push_back(val(x));
    }
  ...
  Tangent der(const GroupScalar x) const
  {
    ...
    if (flag == 0) {
      ...
      Tangent omegaVal;
      if constexpr (Dimension == 1) {
        omegaDer = evalPolyDer<GroupScalar>(x, pieceIdx, n, coefLists_[0], orders_[0]);
        omegaVal = evalPoly<GroupScalar>(x, pieceIdx, coefLists_[0], orders_[0]);
      } else {
        for (std::size_t i = 0; i < Dimension; i++) {
          omegaDer[i] = evalPolyDer<GroupScalar>(x, pieceIdx, n, coefLists_[i], orders_[i]);
          omegaVal[i] = evalPoly<GroupScalar>(x, pieceIdx, coefLists_[i], orders_[i]);
        }
      }

      const auto Dexp = lie::dr_exp<Group>(omegaVal);
      return Dexp * omegaDer;
    } else {
      return extrapPolyDerLie(flag);
    }
  }

  template<typename Derived>
  auto ders(const Eigen::MatrixBase<Derived> & xs) const
  {
    constexpr std::size_t n = 1;

    static_assert(Derived::IsVectorAtCompileTime, "Input must be a vector.");

    Eigen::Matrix<GroupScalar, Dimension, Derived::SizeAtCompileTime> out(Dimension, xs.size());

    for (Eigen::Index i = 0; i < xs.size(); i++) {
      out.col(i) = der(xs[i], n);
    }

    return out;
  }

  vector_aligned<Tangent> ders(const std::vector<GroupScalar> & xs) const
  {
    constexpr std::size_t n = 1;
    vector_aligned<Tangent> out;
    out.reserve(xs.size());
    for (const auto & x : xs) {
      out.push_back(der(x, n));
    }
    return out;
  }
...
protected:
    ...
    const GroupScalar x) const
  {
    if (direction == 1) {
      switch (extrap_) {
        ...
        case EXTRAP::LINEAR: {
            return poses_.back() * Group::exp(extremities_.back * (x - breakPoints_[length_ - 1]));
          }
      ...
      switch (extrap_) {
        ...
        case EXTRAP::LINEAR: {
            return poses_.front() * Group::exp(extremities_.front * (x - breakPoints_[0]));
          }
  ...
  Tangent extrapPolyDerLie(
    const int8_t direction) const
  {
    if (direction == 1) {
      switch (extrap_) {
        case EXTRAP::CLAMP: {
            return groupTangentZero();
          }

        case EXTRAP::LINEAR: {
            return extremities_.back;
          }
  ...
  Tangent groupTangentZero() const noexcept
  {
    if constexpr (Dimension == 1) {
      return 0.;
    } else {
  ...
  std::vector<matrix_t> coefLists_;
  vector_aligned<Group> poses_;
  std::vector<std::size_t> orders_;
  ExtremitiesLie extremities_;
};
